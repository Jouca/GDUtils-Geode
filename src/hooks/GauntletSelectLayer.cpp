...
struct GauntletParticles {
    std::string particle;
    CCPoint pos;
    CCPoint scale = CCPoint(1.310f, 2.610f);
};
...
struct matjson::Serialize<NewGauntlet> {
    static geode::Result<NewGauntlet> fromJson(matjson::Value const& value) {
        ...
        if (value.contains("gauntlet")) {
            GEODE_UNWRAP_INTO(auto bgPos, value["gauntlet"]["position"].asArray());
            data.bgPos = CCPoint(bgPos.at(0).asDouble().unwrapOrDefault(), bgPos.at(1).asDouble().unwrapOrDefault());
            GEODE_UNWRAP_INTO(data.scaleGauntlet, value["gauntlet"]["scale"].asDouble());
        }
        if (value.contains("particles")) {
            GEODE_UNWRAP_INTO(auto particles, value["particles"].asArray());
            for (auto& particle : particles) {
                GauntletParticles particleData;
                GEODE_UNWRAP_INTO(particleData.particle, particle["string"].asString());
                auto posOpt = particle["position"].asArray();
                if (posOpt.isOk()) {
                    auto pos = posOpt.unwrap();
                    particleData.pos = CCPoint(pos.at(0).asDouble().unwrapOrDefault(), pos.at(1).asDouble().unwrapOrDefault());
                }
                GEODE_UNWRAP_INTO(auto scale, particle["scale"].asArray());
                particleData.scale = CCPoint(scale.at(0).asDouble().unwrapOrDefault(), scale.at(1).asDouble().unwrapOrDefault());
                data.particles.push_back(particleData);
            }
        }
...
class $modify(CustomGauntletSelectLayer, GauntletSelectLayer) {
    void reverseGauntlet(GauntletNode* node) {
        ...
        if (auto label_reward = node->m_gauntletInfoNode->getChildByID("reward-label")) {
            ...
            CCNode* label_reward_shadow = node->m_gauntletInfoNode->getChildByID("reward-shadow-label");
            label_reward_shadow->setPositionY(50);

            if (CCSprite* chest = static_cast<CCSprite*>(node->m_gauntletInfoNode->getChildByID("chest-sprite"))) {
                chest->setPositionY(80);
                chest->setFlipY(true);
            }
            if (CCSprite* chest_shadow = static_cast<CCSprite*>(node->m_gauntletInfoNode->getChildByID("chest-shadow-sprite"))) {
                chest_shadow->setPositionY(80);
                chest_shadow->setFlipY(true);
            }
        }
        if (CCNode* label_progress = node->m_gauntletInfoNode->getChildByID("gauntlet-progress-label")) {
            label_progress->setPositionY(29);
        }
        if (CCNode* label_progress_shadow = node->m_gauntletInfoNode->getChildByID("gauntlet-progress-shadow-label")) {
    ...
    bool init(int p0) {
        ...
        CCPoint bcrPos;
        if (auto node = this->getChildByIDRecursive("bottom-right-corner")) {
            bcrPos = node->getPosition();
        }
...
Thanks for their contributions!
                ...
                node->addChildAtPosition(particle_info, Anchor::Center);
    ...
    void setupGauntlets() {
        ...
        if (res.isErr()) return log::error("Couldn't read particles.json: {}", res.unwrapErr());
        ...
        for (int i = 0; i < m_scrollLayer->m_pages->count(); i++) {
            ...
            for (int j = 0; j < page->count(); j++) {
                ...
                for (auto it = json.begin(); it != json.end(); it++) {
                    auto val = *it;
                    if (!val.getKey() || !val.getKey().has_value()) continue;
                    if (val.getKey().value() == gauntletName) {
                        foundIt = true;
                        auto gauntletDataOpt = matjson::Serialize<NewGauntlet>::fromJson(val);
                        if (gauntletDataOpt.isErr()) {
                            log::warn("Couldn't parse the gauntlet named {}: {}", gauntletName, gauntletDataOpt.unwrapErr());
                            continue;
                        }
                        auto gauntletData = gauntletDataOpt.unwrap();
                        Build<CCSprite>::create(fmt::format("{}"_spr, gauntletData.background).c_str())
                            .pos(bg->getPosition())
                            .scale(gauntletData.scaleBg/scaleFactor)
                            .zOrder(0)
                            .id("gauntlet-background"_spr)
                            .parent(node);

                        if (gauntletName == "Strange") {
                            reverseGauntlet(node);
                        }

                        if (gauntletData.scaleGauntlet != 0) {
                            // Show gauntlets sprite again
                            gauntlet->setVisible(true);
                            gauntlet->setPosition(gauntletData.bgPos);
                            gauntlet->setScale(gauntletData.scaleGauntlet);
                            gauntlet_node->setZOrder(3);
                        }

                        // Particle
                        for (size_t i = 0; i < gauntletData.particles.size(); i++) {
                            auto particleData = gauntletData.particles.at(i);
                            CCParticleSystemQuad* particle = GameToolbox::particleFromString(particleData.particle, NULL, false);
                            node->addChild(particle);
                            particle->setZOrder(1);
                            if (particleData.pos.x == 0 && particleData.pos.y == 0) {
                                particle->setPosition(gauntlet->getPosition());
                            } else {
                                particle->setPosition(particleData.pos);
                            }
                            particle->setScaleX(particleData.scale.x);
                            particle->setScaleY(particleData.scale.y);
                            particle->setID(fmt::format("gauntlet-particles-{}"_spr, i + 1));
                        }
                    } else {
