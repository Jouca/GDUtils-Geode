...
void ProgressBar::setProgress(float percentage) {
    ...
    float realPercentage = (204 / 100) *
                           this->m_pPercentage; // "Result of integer division used in a floating point" ok i know clangd warnings wow
...
int DownloadManager::progress_func(void *, double totalDownload, double downloadAmount, double TotalToUpload,
                                   double NowUploaded) {
    ...
    auto layer = reinterpret_cast<ProgressBar *>(reinterpret_cast<CCLayer *>(reinterpret_cast<CCLayer *>(scene->getChildByTag(
            6942084))->getChildren()->objectAtIndex(0))->getChildByTag(4592));
    if (layer != nullptr) {
...
void DownloadManager::setup() {
    ...
    auto text = cocos2d::CCLabelBMFont::create("Do not close this menu while the file is being downloaded.",
                                               "chatFont.fnt");
    text->setPosition(
            winSize.width / 2,
            winSize.height / 2 - 18
    ...
    web::AsyncWebRequest()
            ...
            .expect([](std::string const &err) {
                FLAlertLayer::create(nullptr, "Error!",
                                     "An error occured while trying to send a request to the servers. Check <cy>logs</c> for more information.",
                                     "OK", nullptr, 200.0F)->show();
...
DownloadManager *DownloadManager::create(const char *url, const char *destination, cocos2d::SEL_MenuHandler selector) {
    ...
    if (pRet) {
        ...
        if (pRet->init(DownloadManager::s_defWidth, DownloadManager::s_defHeight, "GJ_square01.png",
                       "Downloading...")) {
            pRet->autorelease();
            return pRet;
        }
