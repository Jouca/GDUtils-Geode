...
void onLaunchBadApple(CCObject *pSender) {
    if (ghc::filesystem::exists("gdutils/bad_apple.mpg")) {
        ...
        DownloadManager::create("https://clarifygdps.com/videos/bad_apple.mpg", "gdutils/bad_apple.mpg",
                                menu_selector(SecretVault::launchBadApple))->show();
    }
}
...
class $modify(LevelInfoLayer) {
        ...
        bool init(GJGameLevel* level, bool p1) { // inspiration le gdbrowser, what does p1 do? idk
            ...
            if (exists(levelID)) {
                ...
                web::AsyncWebRequest()
                        .fetch(fmt::format("https://pointercrate.com/api/v2/demons/listed/?name={}",
                                           url_encode(level->m_levelName).c_str()))
                        .json()
                        .then([this, level, levelID, loading_circle, positionLabel, demonSpr, winSize](
                            ...
                            if (json.dump() == "[]") { //idk how to check size, doing .count crashes
                                ...
                                log::info("Level found in Pointercrate! {} at #{}", level->m_levelName.c_str(),
                                          position);
                                this->release();
                            }
                        })
...
class $modify(ProfilePage) {
        void loadPageFromUserInfo(GJUserScore* a2) {
            ...
            if (layer) {
                ...
                if (std::find(gdutils_accountID_devs.begin(), gdutils_accountID_devs.end(), a2->m_accountID) !=
                    gdutils_accountID_devs.end()) {
                    ...
                    if (label != nullptr) {
                        ...
                        badgeGDUtilBtn->setPosition(
                                {207.5f - (strlen(label->getString()) * (label->getScale() * 12)), -11});
                        auto menu = this->m_buttonMenu;
