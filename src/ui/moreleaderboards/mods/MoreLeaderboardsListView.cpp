#include "MoreLeaderboardsListView.hpp"
...
CCRect MoreLeaderboardsListView::calculateNodeCoverage(CCArray* nodes) {
    CCRect coverage;
    for (auto* childObj : CCArrayExt<CCNode*>(nodes)) {
        CCNode* child = childObj;
        if (!child) continue;
        // only process MoreLeaderboardsCell entries and plain CCNode spacers
        if (!typeinfo_cast<MoreLeaderboardsCell*>(child) && child->getChildrenCount() > 0) continue;
        ...
        if (pos.x < coverage.origin.x) coverage.origin.x = pos.x;
        if (pos.y < coverage.origin.y) coverage.origin.y = pos.y;
        if (csize.x > coverage.size.width) coverage.size.width = csize.x;
        if (csize.y > coverage.size.height) coverage.size.height = csize.y;
    }
    return coverage;
}
