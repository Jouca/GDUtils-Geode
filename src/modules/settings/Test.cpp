...
class TestValue : public SettingV3 {
public:
    static Result<std::shared_ptr<SettingV3>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
        auto res = std::make_shared<TestValue>();
        auto root = checkJson(json, "TestValue");
...
class TestNode : public SettingNodeV3 {
protected:
    bool init(std::shared_ptr<TestValue> setting, float width) {
        if (!SettingNodeV3::init(setting, width))
            return false;
        this->setContentSize({ width, 35.f });
        getButtonMenu()->setContentWidth(width);
        Build<CCLabelBMFont>::create("Test", "bigFont.fnt").scale(0.6f).parentAtPos(getButtonMenu(), Anchor::Left, {42, 0});
        Build<CCSprite>::createSpriteName("GJ_playBtn2_001.png").scale(0.4f).intoMenuItem([]() {
        ...
        }).parentAtPos(getButtonMenu(), Anchor::Right, {-15, 0});
...
public:
    ...
    static TestNode* create(std::shared_ptr<TestValue> setting, float width) {
    ...
    std::shared_ptr<TestValue> getSetting() const {
        return std::static_pointer_cast<TestValue>(SettingNodeV3::getSetting());
    }
};

SettingNodeV3* TestValue::createNode(float width) {
    return TestNode::create(static_pointer_cast<TestValue>(shared_from_this()), width);
}

$execute {
    (void)Mod::get()->registerCustomSettingType("test-notification", &TestValue::parse);
}
