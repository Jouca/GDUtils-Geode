// Spotify
// sorry this code will forever be bad
bool is_muted = false;
...
class $modify(PlayLayer) {
    bool init(GJGameLevel* level, bool p1, bool p2) {
        ...
        if (FMODAudioEngine::sharedEngine()->m_musicVolume > 0.0F) {
...
class $modify(PauseLayer) {
    ...
    void goEdit() {
        ...
        if (FMODAudioEngine::sharedEngine()->m_musicVolume > 0.0F) { // temp until someone adds fields
            toggleSpotifyMute();
        }
    }
};
...
void toggleHook(bool level, bool value) {
    for (auto& hook : geode::Mod::get()->getHooks()) {
        if (level && !hook->getDisplayName().starts_with("PauseLayer") && !hook->getDisplayName().starts_with("PlayLayer")) continue;
        if (!level && !hook->getDisplayName().starts_with("EditorUI")) continue;
        if (value) {
            (void)hook->enable();
        } else {
            (void)hook->disable();
...
$execute {
    listenForSettingChanges<bool>("inLevelsSpotify", [](bool value) {
        log::info("Spotify In Level changed to {}", value);
        toggleHook(true, value);
    });
    listenForSettingChanges<bool>("inEditorSpotify", [](bool value) {
        log::info("Spotify In Editor changed to {}", value);
        toggleHook(false, value);
    });
}

$on_mod(Loaded) {
    if (!Mod::get()->getSettingValue<bool>("inLevelsSpotify")) {
        toggleHook(true, false);
    }
    if (!Mod::get()->getSettingValue<bool>("inEditorSpotify")) {
        toggleHook(false, false);
    }
}
